...
public class BigHotblingTime extends TimerTask {
	...
	public void run() {
		...
		case 7284:
			...
			World.get().broadcastPacketToAll(
					new S_SystemMessage("大樂透 第 " + String.valueOf(BigHotId) + " 期開出的號碼是 " + this.BigHotAN + "。"));
		}
	...
	private void toRate(int type, int info) {
		for (L1Object object : World.get().getObject()) {
			...
			if (npc.getNpcTemplate().get_npcId() == L1Config_other._2162) {
				String Name = null;
				switch (type) {
				case 1:
					Name = "一";
	...
	private void toAllTimeS(String info) {
		for (L1Object object : World.get().getObject()) {
			if (!(object instanceof L1NpcInstance))
				continue;
			L1NpcInstance npc = (L1NpcInstance) object;
			if ((npc.getNpcTemplate().get_npcId() == L1Config_other._2162) && (npc != null)) {
	...
	private void checkVictory() {
		for (L1Object object : World.get().getObject()) {
			if (!(object instanceof L1NpcInstance))
				continue;
			L1NpcInstance npc = (L1NpcInstance) object;
			if ((npc.getNpcTemplate().get_npcId() == L1Config_other._2162) && (this.BigHotAN != null)) {
				int BigHotId = this._BigHot.get_BigHotId();
	...
	private void isOver() {
		...
		BigHotblingLock.create().create(this._BigHot.get_BigHotId(), this.BigHotAN, yuanbao, yuanbao1, count1, yuanbao2,
				count2, yuanbao3, count3, count4);
